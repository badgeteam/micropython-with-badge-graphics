...
static mp_obj_t framebuffer_png_info(mp_uint_t n_args, const mp_obj_t *args)
{
	...
	if(!MP_OBJ_IS_TYPE(args[0], &mp_type_bytes)) {
        mp_obj_new_exception_msg_varg(&mp_type_ValueError, "Expected a bytes object");
        return mp_const_none;
    }
...
static mp_obj_t framebuffer_draw_png(mp_uint_t n_args, const mp_obj_t *args)
{
    ...
    if(!MP_OBJ_IS_TYPE(args[paramOffset], &mp_type_bytes)) {
        mp_obj_new_exception_msg_varg(&mp_type_ValueError, "Expected a bytes object");
        return mp_const_none;
    }
...
const mp_obj_module_t mp_module_framebuffer = {
	.base = {&mp_type_module},
